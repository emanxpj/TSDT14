...
described in equation~\ref{eq:idealH} to produce the signal $x$.
...
This signal, $x$, is then used as input to the three different non-LTI-systems described in equation~\ref{eq:SQsys},
~\ref{eq:HWsys} and ~\ref{eq:AMsys}. This gives the output signal $y$.
...
\subsubsection{Theoretical PSD}

\subsubsection{Amplitude distribution and estimated PSD}
The amplitude distribution is computed by using a histogram and the PSD is estimated using the same methods
as in study 1.
